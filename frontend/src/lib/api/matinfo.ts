...
export const matinfoApi = {
  ...
  list: async (params?: MatinfoListParams): Promise<MatinfoProductListResponse> => {
    ...
    const response = await apiClient.get(`/v1/matinfo/products?${queryParams}`)
    return response.data
  },

  // Get single product by ID
  get: async (id: string): Promise<MatinfoProduct> => {
    const response = await apiClient.get(`/v1/matinfo/products/${id}`)
    return response.data
  },

  // Get product by GTIN
  getByGtin: async (gtin: string): Promise<MatinfoProduct> => {
    const response = await apiClient.get(`/v1/matinfo/products/gtin/${gtin}`)
    return response.data
  },

  // Create product
  create: async (data: Partial<MatinfoProduct>): Promise<MatinfoProduct> => {
    const response = await apiClient.post('/v1/matinfo/products', data)
    return response.data
  },

  // Update product
  update: async (id: string, data: Partial<MatinfoProduct>): Promise<MatinfoProduct> => {
    const response = await apiClient.put(`/v1/matinfo/products/${id}`, data)
    return response.data
  },

  // Delete product
  delete: async (id: string): Promise<void> => {
    await apiClient.delete(`/v1/matinfo/products/${id}`)
  },

  // Search products
  search: async (query: string, limit = 20): Promise<MatinfoProduct[]> => {
    const response = await apiClient.get(`/v1/matinfo/products/search?q=${encodeURIComponent(query)}&limit=${limit}`)
    return response.data.items || []
  },

  // Get linked LKC product by GTIN
  getLinkedProduct: async (gtin: string): Promise<LinkedProductResponse> => {
    const response = await apiClient.get(`/v1/matinfo/products/gtin/${gtin}/linked`)
    return response.data
  },
  ...
  sync: {
    ...
    syncProducts: async (daysBack = 7): Promise<SyncResponse> => {
      const response = await apiClient.post(`/v1/matinfo-sync/sync/products?days_back=${daysBack}`)
      return response.data
    },

    // Sync single product by GTIN
    syncProduct: async (gtin: string): Promise<{ message: string; gtin: string }> => {
      const response = await apiClient.post(`/v1/matinfo-sync/sync/product/${gtin}`)
      return response.data
    },

    // Get list of updated GTINs
    getUpdatedGtins: async (daysBack = 7): Promise<GTINListResponse> => {
      const response = await apiClient.get(`/v1/matinfo-sync/sync/updated-gtins?days_back=${daysBack}`)
      return response.data
    },

    // Get new products (in Matinfo but not in our DB)
    getNewProducts: async (daysBack = 30): Promise<GTINListResponse> => {
      const response = await apiClient.get(`/v1/matinfo-sync/sync/new-products?days_back=${daysBack}`)
      return response.data
    },

    // Search by name using AI variations
    searchByName: async (name: string, limit = 10): Promise<ProductSearchResponse> => {
      const response = await apiClient.post(`/v1/matinfo-sync/search/name?name=${encodeURIComponent(name)}&limit=${limit}`)
      return response.data
    },
  },
  ...
  vendorImport: async (data: Partial<MatinfoProduct>): Promise<VendorImportResponse> => {
    const response = await apiClient.post('/v1/matinfo/products/vendor-import', data)
    return response.data
  },

  // Batch vendor import
  vendorImportBatch: async (products: Partial<MatinfoProduct>[]): Promise<VendorImportResponse[]> => {
    const response = await apiClient.post('/v1/matinfo/products/vendor-import/batch', products)
    return response.data
  },
  ...
  allergens: {
    update: async (productId: string, code: string, data: { level: string; name: string }): Promise<Allergen> => {
      const response = await apiClient.put(`/v1/matinfo/products/${productId}/allergens/${code}`, data)
      return response.data
    },
    delete: async (productId: string, code: string): Promise<void> => {
      await apiClient.delete(`/v1/matinfo/products/${productId}/allergens/${code}`)
    },
  },
  ...
  nutrients: {
    update: async (productId: string, code: string, data: Partial<Nutrient>): Promise<Nutrient> => {
      const response = await apiClient.put(`/v1/matinfo/products/${productId}/nutrients/${code}`, data)
      return response.data
    },
    delete: async (productId: string, code: string): Promise<void> => {
      await apiClient.delete(`/v1/matinfo/products/${productId}/nutrients/${code}`)
    },
  },
}
